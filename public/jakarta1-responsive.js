(()=>{
  ...
  const addStyles=()=>{
    ...
    s.textContent=`
      ...
      @media(max-width:1023px){
        ...
        body>div:first-child aside,#__next aside,aside{position:fixed!important;inset:0 auto 0 0!important;width:min(86vw,320px)!important;height:100dvh!important;z-index:80!important;overflow-y:auto!important;transform:translateX(-105%);transition:transform .22s ease;box-shadow:0 20px 50px rgba(15,23,42,.28)}
        body.j1-menu-open aside{transform:translateX(0)}
        ...
        main>div{max-width:100%;overflow-x:hidden}
  ...
  const setup=()=>{
    ...
    if(!nav.querySelector('[data-j1-dashboard-label]')){
      const label=document.createElement('div');label.className='j1-nav-group';label.dataset.j1DashboardLabel='1';label.textContent='Dashboard';nav.prepend(label);
    }
    const target=[...nav.querySelectorAll('button')].find(b=>b.textContent?.trim().startsWith('Target Fokus'));
    if(target&&!nav.querySelector('[data-j1-target-label]')){
      const host=target.parentElement;
      if(host){const label=document.createElement('div');label.className='j1-nav-group';label.dataset.j1TargetLabel='1';label.textContent='Target Fokus';host.before(label)}
    }
